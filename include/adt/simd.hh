/* 
 * <mmintrin.h>  MMX
 * <xmmintrin.h> SSE
 * <emmintrin.h> SSE2
 * <pmmintrin.h> SSE3
 * <tmmintrin.h> SSSE3
 * <smmintrin.h> SSE4.1
 * <nmmintrin.h> SSE4.2
 * <ammintrin.h> SSE4A
 * <wmmintrin.h> AES
 * <immintrin.h> AVX, AVX2, FMA
 */

#pragma once

#include "Span.hh" /* IWYU pragma: keep */
#include "math-inl.hh"

#ifdef ADT_SSE4_2
    #include <nmmintrin.h>
#endif

#if defined ADT_AVX2
    #include <immintrin.h>
#endif

namespace adt::simd
{

#ifdef ADT_SSE4_2

struct f32x4;

struct i32x4
{
    __m128i pack {};

    /* */

    i32x4() = default;

    explicit i32x4(const __m128i _pack) : pack(_pack) {}

    explicit i32x4(i32 x) : pack(_mm_set1_epi32(x)) {}

    i32x4(i32 x, i32 y, i32 z, i32 w) : pack(_mm_set_epi32(w, z, y, x)) {}

    /* */

    explicit operator __m128i() const { return pack; }

    /* */

    decltype(auto) data() { return reinterpret_cast<i32(&)[4]>(*this); }
    decltype(auto) data() const { return reinterpret_cast<const i32(&)[4]>(*this); }

    i32& operator[](int i)             { ADT_ASSERT(i >= 0 && i < 4, "out of range, should be (>= 0 && < 4)"); return data()[i]; }
    const i32& operator[](int i) const { ADT_ASSERT(i >= 0 && i < 4, "out of range, should be (>= 0 && < 4)"); return data()[i]; }

    i32x4 operator+(const i32x4& r) const;
    i32x4 operator-(const i32x4& r) const;
    i32x4& operator+=(const i32x4& r);
    i32x4& operator-=(const i32x4& r);
    i32x4 operator*(const i32x4& r) const;
    i32x4& operator*=(const i32x4 r);
    i32x4 operator|(const i32x4& r) const;
    i32x4 operator&(const i32x4& r) const;
    i32x4 operator>=(const i32x4& r) const;
    i32x4 operator<(const i32x4& r) const;
    i32x4 operator>>(i32 b) const;
    i32x4 operator<<(i32 b) const;
};

struct f32x4
{
    __m128 pack {};

    /* */

    f32x4() = default;

    explicit f32x4(const __m128 _pack) : pack(_pack) {}
    explicit f32x4(const __m128i _pack) : pack(_mm_cvtepi32_ps(_pack)) {}

    explicit f32x4(const f32 x) : pack(_mm_set1_ps(x)) {}

    f32x4(f32 x, f32 y, f32 z, f32 w) : pack(_mm_set_ps(w, z, y, x)) {}

    f32x4(const math::V4 v) : pack(_mm_set_ps(v.w, v.z, v.y, v.x)) {}

    explicit f32x4(i32x4 x) : pack {_mm_cvtepi32_ps(x.pack)} {}

    /* */

    explicit operator __m128() const { return pack; }

    explicit operator math::V4() const { return *reinterpret_cast<const math::V4*>(this); }

    explicit operator math::Qt() const { return reinterpret_cast<const math::Qt&>(*this); }

    explicit operator i32x4() const { return i32x4{_mm_cvtps_epi32(pack)}; }

    decltype(auto) data() { return reinterpret_cast<f32(&)[4]>(*this); }
    decltype(auto) data() const { return reinterpret_cast<const f32(&)[4]>(*this); }

    f32& operator[](int i)             { ADT_ASSERT(i >= 0 && i < 4, "out of range, should be (>= 0 && < 4)"); return data()[i]; }
    const f32& operator[](int i) const { ADT_ASSERT(i >= 0 && i < 4, "out of range, should be (>= 0 && < 4)"); return data()[i]; }

    f32x4 operator+(const f32x4& r) const;
    f32x4 operator-(const f32x4& r) const;
    f32x4& operator+=(const f32x4& r);
    f32x4& operator-=(const f32x4& r);
    f32x4 operator*(const f32x4& r) const;
    f32x4 operator*(const f32& r) const;
    f32x4 operator/(const f32x4& r) const;
    f32x4 operator&(const f32x4& r) const;
    f32x4 operator-() const;
    f32x4 operator<(const f32x4& r) const;
};

inline i32x4
i32x4Reinterpret(const f32x4 x)
{
    return i32x4{_mm_castps_si128(x.pack)};
}

inline f32x4
f32x4Reinterpret(const i32x4 x)
{
    return f32x4{_mm_castsi128_ps(x.pack)};
}

inline i32x4
i32x4Load(const i32* const ptr)
{
    return i32x4{_mm_loadu_si128(reinterpret_cast<const __m128i*>(ptr))};
}

inline i32x4
i32x4LoadI32x2(const i64 _i32x2)
{
    return i32x4{_mm_cvtsi64_si128(_i32x2)};
}

inline i32x4
i32x4Load(const __m128i* const ptr)
{
    return i32x4{_mm_loadu_si128(ptr)};
}

inline i32x4
i32x4Gather(i32* const p, const i32x4 offSets)
{
    return {p[ offSets[0] ], p[ offSets[1] ], p[ offSets[2] ], p[ offSets[3] ]};
}

inline f32x4
f32x4Load(const f32* const ptr)
{
    return f32x4{_mm_loadu_ps(ptr)};
}

inline void
f32x4Store(f32* const pDest, const f32x4 x)
{
    _mm_storeu_ps(pDest, x.pack);
}

inline void
i32x4Store(i32* const pDest, const i32x4 x)
{
    _mm_storeu_si128(reinterpret_cast<__m128i*>(pDest), x.pack);
}

inline void
i32x4Store(__m128i* const pDest, const i32x4 x)
{
    _mm_storeu_si128(pDest, x.pack);
}

inline i32x4
i32x4::operator+(const i32x4& r) const
{
    return i32x4{_mm_add_epi32(pack, r.pack)};
}

inline f32x4
f32x4::operator+(const f32x4& r) const
{
    return f32x4{_mm_add_ps(pack, r.pack)};
}

inline f32x4
f32x4::operator-(const f32x4& r) const
{
    return f32x4{_mm_sub_ps(pack, r.pack)};
}

inline f32x4&
f32x4::operator+=(const f32x4& r)
{
    return *this = *this + r;
}

inline f32x4&
f32x4::operator-=(const f32x4& r)
{
    return *this = *this - r;
}

inline i32x4
i32x4::operator-(const i32x4& r) const
{
    return i32x4{_mm_sub_epi32(pack, r.pack)};
}

inline i32x4&
i32x4::operator+=(const i32x4& r)
{
    return *this = *this + r;
}

inline i32x4&
i32x4::operator-=(const i32x4& r)
{
    return *this = *this - r;
}

inline i32x4
i32x4::operator*(const i32x4& r) const
{
    return i32x4{_mm_mullo_epi32(pack, r.pack)};
}

inline i32x4&
i32x4::operator*=(const i32x4 r)
{
    return *this = *this * r;
}

inline f32x4
f32x4::operator*(const f32x4& r) const
{
    return f32x4{_mm_mul_ps(pack, r.pack)};
}

inline f32x4
f32x4::operator*(const f32& r) const
{
    return *this * f32x4(r);
}

inline f32x4
f32x4::operator/(const f32x4& r) const
{
    return f32x4{_mm_div_ps(pack, r.pack)};
}

inline i32x4
i32x4::operator|(const i32x4& r) const
{
    return i32x4{_mm_or_si128(pack, r.pack)};
}

inline i32x4
i32x4::operator&(const i32x4& r) const
{
    return i32x4{_mm_and_si128(pack, r.pack)};
}

inline f32x4
f32x4::operator&(const f32x4& r) const
{
    return f32x4{_mm_and_ps(pack, r.pack)};
}

inline f32x4
f32x4::operator-() const
{
    return *this * f32x4(-1.0f);
}

inline f32x4
f32x4::operator<(const f32x4& r) const
{
    return f32x4{_mm_cmplt_ps(pack, r.pack)};
}

inline i32x4
i32x4::operator>=(const i32x4& r) const
{
    return i32x4{_mm_cmpgt_epi32(pack, r.pack) | _mm_cmpeq_epi32(pack, r.pack)};
}

inline i32x4
i32x4::operator<(const i32x4& r) const
{
    return i32x4{_mm_cmplt_epi32(pack, r.pack)};
}

inline i32x4
i32x4::operator>>(i32 b) const
{
    return i32x4{_mm_srli_epi32(pack, b)};
}

inline i32x4
i32x4::operator<<(i32 b) const
{
    return i32x4{_mm_slli_epi32(pack, b)};
}

inline i32x4
andNot(const i32x4& mask, const i32x4& x)
{
    return i32x4{_mm_andnot_si128(mask.pack, x.pack)};
}

inline f32x4
andNot(const f32x4& mask, const f32x4& x)
{
    return f32x4{_mm_andnot_ps(mask.pack, x.pack)};
}

inline int
moveMask8(const i32x4& x)
{
    return _mm_movemask_epi8(x.pack);
}

inline i32x4
min(const i32x4& l, const i32x4& r)
{
    return i32x4{_mm_min_epi32(l.pack, r.pack)};
}

inline i32x4
max(const i32x4& l, const i32x4& r)
{
    return i32x4{_mm_max_epi32(l.pack, r.pack)};
}

inline f32x4
floor(const f32x4& x)
{
    return f32x4{_mm_floor_ps(x.pack)};
}

inline void
i32Fillx4(Span<i32> src, const i32 x)
{
    const i32 pack = x;

    isize i = 0;
    for (; i + 3 < src.size(); i += 4)
        i32x4Store(&src[i], i32x4{pack});

    for (; i < src.size(); ++i)
        src[i] = x;
}

inline void
f32Fillx4(Span<f32> src, const f32 x)
{
    f32x4 pack {x};

    isize i = 0;
    for (; i + 3 < src.size(); i += 4)
        f32x4Store(&src[i], pack);

    for (; i < src.size(); ++i)
        src[i] = x;
}

inline void
i16Fillx8(Span<i16> src, const i16 x)
{
    __m128i pack = _mm_set1_epi16(x);
    isize i = 0;
    for (; i + 7 < src.size(); i += 8)
        _mm_storeu_si128(reinterpret_cast<__m128i*>(&src[i]), pack);

    for (; i < src.size(); ++i)
        src[i] = x;
}

#endif /* ADT_SSE4_2 */

#if defined ADT_AVX2

struct f32x8;

struct i32x8
{
    __m256i pack {};

    /* */

    i32x8() = default;

    explicit i32x8(const __m256i _pack) : pack(_pack) {}

    explicit i32x8(const i32 x) : pack(_mm256_set1_epi32(x)) {}

    i32x8(i32 x0, i32 x1, i32 x2, i32 x3, i32 x4, i32 x5, i32 x6, i32 x7)
        : pack(_mm256_set_epi32(x7, x6, x5, x4, x3, x2, x1, x0)) {}

    /* */

    explicit operator __m256i() const { return pack; }

    explicit operator f32x8() const;

    /* */

    decltype(auto) data() { return reinterpret_cast<i32(&)[8]>(*this); }
    decltype(auto) data() const { return reinterpret_cast<const i32(&)[8]>(*this); }

    i32& operator[](int i)             { ADT_ASSERT(i >= 0 && i < 8, "out of range, should be (>= 0 && < 8) got: {}", i); return data()[i]; }
    const i32& operator[](int i) const { ADT_ASSERT(i >= 0 && i < 8, "out of range, should be (>= 0 && < 8) got: {}", i); return data()[i]; }

    i32x8 operator+(const i32x8& r) const;
    i32x8 operator-(const i32x8& r) const;
    i32x8& operator+=(const i32x8& r);
    i32x8& operator-=(const i32x8& r);
    i32x8 operator*(const i32x8& r) const;
    i32x8& operator*=(const i32x8& r);
    i32x8 operator|(const i32x8& r) const;
    i32x8 operator&(const i32x8& r) const;
    i32x8 operator>=(const i32x8& r) const;
    i32x8 operator<(const i32x8& r) const;
    i32x8 operator<<(const i32 b) const;
    i32x8 operator>>(const i32 b) const;
};

struct f32x8
{
    __m256 pack {};

    /* */

    f32x8() = default;

    f32x8(const __m256 _pack) : pack(_pack) {}
    f32x8(const __m256i _pack) : pack(_mm256_cvtepi32_ps(_pack)) {}

    f32x8(const f32 x) : pack(_mm256_set1_ps(x)) {}

    f32x8(i32x8 _pack) : pack {_mm256_cvtepi32_ps(_pack.pack)} {}

    /* */

    decltype(auto) data() { return reinterpret_cast<f32(&)[8]>(*this); }
    decltype(auto) data() const { return reinterpret_cast<const f32(&)[8]>(*this); }

    f32& operator[](int i)             { ADT_ASSERT(i >= 0 && i < 8, "out of range, should be (>= 0 && < 8) got: {}", i); return data()[i]; }
    const f32& operator[](int i) const { ADT_ASSERT(i >= 0 && i < 8, "out of range, should be (>= 0 && < 8) got: {}", i); return data()[i]; }

    explicit operator __m256() const { return pack; }

    explicit operator i32x8() const { return i32x8{_mm256_cvtps_epi32(pack)}; }

    f32x8 operator+(const f32x8& r) const;
    f32x8 operator-(const f32x8& r) const;
    f32x8& operator+=(const f32x8& r);
    f32x8& operator-=(const f32x8& r);
    f32x8 operator*(const f32x8& r) const;
    f32x8 operator*(const f32 r) const;
    f32x8 operator/(const f32x8& r) const;
    f32x8 operator&(const f32x8& r) const;
    f32x8 operator-() const;
    f32x8 operator<(const f32x8& r) const;
};

inline i32x8
i32x8Reinterpret(const f32x8 x)
{
    return i32x8{_mm256_castps_si256(x.pack)};
}

inline f32x8
f32x8Reinterpret(const i32x8 x)
{
    return _mm256_castsi256_ps(x.pack);
}

inline
i32x8::operator f32x8() const
{
    return _mm256_cvtepi32_ps(pack);
}

inline i32x8
i32x8Load(const i32* const ptr)
{
    return i32x8{_mm256_loadu_si256(reinterpret_cast<const __m256i*>(ptr))};
}

inline i32x8
i32x8Load(const __m256i* const ptr)
{
    return i32x8{_mm256_loadu_si256(ptr)};
}

inline i32x8
i32x8Gather(i32* const p, const i32x8 offSets)
{
    return {
        p[ offSets[0] ], p[ offSets[1] ], p[ offSets[2] ], p[ offSets[3] ],
        p[ offSets[4] ], p[ offSets[5] ], p[ offSets[6] ], p[ offSets[7] ],
    };
}

inline f32x8
f32x8Load(const f32* const ptr)
{
    return _mm256_loadu_ps(ptr);
}

inline void
f32x8Store(f32* const pDest, const f32x8 x)
{
    _mm256_storeu_ps(pDest, x.pack);
}

inline void
i32x8Store(i32* const pDest, const i32x8 x)
{
    _mm256_storeu_si256(reinterpret_cast<__m256i*>(pDest), x.pack);
}

inline void
i32x8Store(__m256i* const pDest, const i32x8 x)
{
    _mm256_storeu_si256(pDest, x.pack);
}

inline i32x8
i32x8::operator+(const i32x8& r) const
{
    return i32x8{_mm256_add_epi32(pack, r.pack)};
}

inline f32x8
f32x8::operator+(const f32x8& r) const
{
    return _mm256_add_ps(pack, r.pack);
}

inline f32x8
f32x8::operator-(const f32x8& r) const
{
    return _mm256_sub_ps(pack, r.pack);
}

inline f32x8&
f32x8::operator+=(const f32x8& r)
{
    return *this = *this + r;
}

inline f32x8&
f32x8::operator-=(const f32x8& r)
{
    return *this = *this - r;
}

inline i32x8
i32x8::operator-(const i32x8& r) const
{
    return i32x8{_mm256_sub_epi32(pack, r.pack)};
}

inline i32x8&
i32x8::operator+=(const i32x8& r)
{
    return *this = *this + r;
}

inline i32x8&
i32x8::operator-=(const i32x8& r)
{
    return *this = *this - r;
}

inline i32x8
i32x8::operator*(const i32x8& r) const
{
    return i32x8{_mm256_mullo_epi32(pack, r.pack)};
}

inline i32x8&
i32x8::operator*=(const i32x8& r)
{
    return *this = *this * r;
}

inline f32x8
f32x8::operator*(const f32x8& r) const
{
    return _mm256_mul_ps(pack, r.pack);
}

inline f32x8
f32x8::operator*(const f32 r) const
{
    return *this * f32x8(r);
}

inline f32x8
f32x8::operator/(const f32x8& r) const
{
    return _mm256_div_ps(pack, r.pack);
}

inline i32x8
i32x8::operator|(const i32x8& r) const
{
    return i32x8{_mm256_or_si256(pack, r.pack)};
}

inline i32x8
i32x8::operator&(const i32x8& r) const
{
    return i32x8{_mm256_and_si256(pack, r.pack)};
}

inline f32x8
f32x8::operator&(const f32x8& r) const
{
    return _mm256_and_ps(pack, r.pack);
}

inline f32x8
f32x8::operator-() const
{
    return *this * f32x8(-1.0f);
}

inline f32x8
f32x8::operator<(const f32x8& r) const
{
    return _mm256_cmp_ps(pack, r.pack, _CMP_LT_OQ);
}

inline i32x8
i32x8::operator>=(const i32x8& r) const
{
    return i32x8{_mm256_cmpgt_epi32(pack, r.pack) | _mm256_cmpeq_epi32(pack, r.pack)};
}

inline i32x8
i32x8::operator<(const i32x8& r) const
{
    /* in reverse */
    return i32x8{_mm256_cmpgt_epi32(r.pack, pack)};
}

inline i32x8
i32x8::operator<<(const i32 b) const
{
    return i32x8 {_mm256_slli_epi32(pack, b)};
}

inline i32x8
i32x8::operator>>(const i32 b) const
{
    return i32x8{_mm256_srli_epi32(pack, b)};
}

inline i32x8
andNot(const i32x8& mask, const i32x8& x)
{
    return i32x8{_mm256_andnot_si256(mask.pack, x.pack)};
}

inline f32x8
andNot(const f32x8& mask, const f32x8& x)
{
    return _mm256_andnot_ps(mask.pack, x.pack);
}

inline int
moveMask8(const i32x8& x)
{
    return _mm256_movemask_epi8(x.pack);
}

inline i32x8
min(const i32x8& l, const i32x8& r)
{
    return i32x8{_mm256_min_epi32(l.pack, r.pack)};
}

inline i32x8
max(const i32x8& l, const i32x8& r)
{
    return i32x8{_mm256_max_epi32(l.pack, r.pack)};
}

inline f32x8
floor(const f32x8& x)
{
    return _mm256_floor_ps(x.pack);
}

inline void
i32Fillx8(Span<i32> src, const i32 x)
{
    const i32 pack = x;

    isize i = 0;
    for (; i + 7 < src.size(); i += 8)
        i32x8Store(&src[i], i32x8{pack});

    for (; i < src.size(); ++i)
        src[i] = x;
}

inline void
f32Fillx8(Span<f32> src, const f32 x)
{
    f32x8 pack = x;

    isize i = 0;
    for (; i + 7 < src.size(); i += 8)
        f32x8Store(&src[i], pack);

    for (; i < src.size(); ++i)
        src[i] = x;
}

inline f32x4
fma(const f32x4& a, f32 b, const f32x4& c)
{
    return f32x4{_mm_fmadd_ps(a.pack, f32x4(b).pack, c.pack)};
}

#endif /* ADT_AVX2 */

} /* namespace adt::simd */
